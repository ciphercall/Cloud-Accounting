...
namespace ASGL.Controllers.Calendar
{
    public class PromotionalCalendarUploadController : AppController
    {
            ...
            try
            {
                if (file.ContentLength > 0)
                {
                    ...
                    var findPreviousImages =
                        (from m in db.PCalendarImageDbSet where m.Year == model.Year && m.Month == model.Month select m)
                            .ToList();
                    ...
                    {
                        db.PCalendarImageDbSet.Add(model);
                        db.SaveChanges();
                    }
        ...
        public PartialViewResult CalendarInfo(Int64 year)
        {
            ...
            calendarImages = db.PCalendarImageDbSet.Where(e => e.Year == year).OrderBy(e => e.Month).ToList();
